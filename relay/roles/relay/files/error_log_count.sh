...
# DataDogへ送信(/var/log/message)追加
# エラー数を取得（行数）
error_count=`grep "Fatal" /var/log/messages |wc -l`
curl  -X POST -H "Content-type: application/json" \
-d "{ \"series\" :
         [{\"metric\":\"var_log_messages_fatal_count\",
          \"points\":[[${currenttime}, ${error_count}]],
          \"type\":\"gauge\",
          \"host\":\"${hostname}\",
          \"tags\":[\"groups:${groups}\",\"logtype:messages\"]}]
    }" \
'https://app.datadoghq.com/api/v1/series?api_key=0710f2e8e084bce4dc47dabd4364929b'
