...
echo "Starting AzkabanWebServer on port $executorport ..." 
serverpath=`pwd`
...
java $AZKABAN_OPTS $NATIVE_LIB_OPTS -cp $CLASSPATH azkaban.webapp.AzkabanWebServer -conf $base_dir/conf $@ & >> ${log_dir}/azkaban-web.log

echo $! > ${pid_dir}/azkaban-web.pid
